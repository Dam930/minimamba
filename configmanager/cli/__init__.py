...
from configmanager.configs.models import GlobalContextConfig
...
from configmanager.utils.global_context import GlobalContextManager
...
def _create_subparsers(parser: ArgumentParser):
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=metadata.version("configmanager"),
    )
    parser.add_argument(
        "-g",
        "--global-config",
    ...
    # each file in "configmanager/cli" defines a CLI entry-point for a command
...
def _add_subparser_from_file(filepath: Path, subparsers: list):
    ...
    callback_module = f"configmanager.commands.{filepath.stem}"
...
def _create_global_context(path_global_config: Optional[str], command_name: str):
    ...
    if path_global_config is not None:
        global_config = ConfigReader(
            Path(path_global_config), "configmanager.configs.models"
        ).get_config()
    else:
        global_config = GlobalContextConfig(
            __config_class=".".join(
