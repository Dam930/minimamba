...
project_root = Path(".")
file_toml = project_root / "pyproject.toml"
...
def fill_pyproject_toml(
    description: str, keywords: str, author: str, author_email: str
) -> None:
    lines = []
    with open(file_toml, "r") as f:
        for line in f:
            line = line.rstrip()

            if 'description = "An Artificialy SA project"' in line:
                line = line.replace(
                    "An Artificialy SA project",
                    description,
                )
            if '{name="Artificialy SA", email="info@artificialy.com"}' in line:
                line = line.replace(
                    "Artificialy SA",
                    author,
                )
            ...
            if 'keywords = ["python", "machine-learning"]' in line:
                keywords = [f"{k}" for k in keywords.split(",")]
                line = line.replace(
...
def update_python_version(python_version: str) -> None:
    min_python_minor = int(python_version.split(".")[1])
    max_python_minor = min_python_minor + 1

    _update_pyver_in_toml(
        min_python_minor=min_python_minor, max_python_minor=max_python_minor
    )

    _update_pyver_in_readme(
        min_python_minor=min_python_minor, max_python_minor=max_python_minor
    )

    _update_pyver_in_ci(min_python_minor)


def _update_pyver_in_toml(min_python_minor: int, max_python_minor: int) -> None:
    lines = []
    with open(file_toml, "r") as f:
        for line in f:
            line = line.rstrip()
            ...
            if line.startswith("requires-python"):
                line = (
                    f'requires-python = ">=3.{min_python_minor}, <3.{max_python_minor}"'
                )
...
def _update_pyver_in_readme(min_python_minor: int, max_python_minor: int):
    ...
    with open(path_readme, "r") as f:
        for line in f:
            ...
            if "Python 3.10" in line:
                line = line.replace("Python 3.10", f"Python 3.{min_python_minor}")
            if "python=3.10" in line:
                line = line.replace("python=3.10", f"python=3.{min_python_minor}")
...
def _update_pyver_in_ci(min_python_minor: int):
    ...
    with open(path_ci, "r") as f:
        for line in f:
            ...
            if 'PYTHON_VERSION: "3.10"' in line:
                line = line.replace("3.10", f"3.{min_python_minor}")
...
def setup_git_repo() -> None:
    ...
    if setup_remote:
        ...
        out.check_returncode()
        out = subprocess.run(["git", "push", "--set-upstream", "origin", "development"])
        out.check_returncode()
    else:
        print(
...
def main() -> None:
    ...
    fill_pyproject_toml(description, keywords, author_name, author_email)
